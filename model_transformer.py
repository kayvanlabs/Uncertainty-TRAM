...
class Block(nn.Module):
    ...
    def forward(self, x, return_attention=False, register_hook=False):
        y, attn = self.attn(self.norm1(x), register_hook=register_hook)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_hook=False, return_attentions=False):
        ...
        for blk in self.blocks:
            if return_attentions:
                x, attns = blk(x, register_hook=register_hook, return_attention=True)
        ...
        if return_attentions:
            return x[:, 0], torch.cat(attentions).unsqueeze(0)
        else:
            return x[:, 0]
