...
class TRAM_Ordinal_Trainer(Trainer):
    """
    Trainer class for the TRAM model that has ordinal regression output 
    """
    def __init__(self, *kwags, **kwargs):
        super().__init__(*kwags, **kwargs)
        ...
        self.train_metrics = module_metric.MetricTracker(
            'loss', 'base_loss', *[m for m in self.metric_ftns], 
            'ordi_loss', writer=self.writer)
        
        self.valid_metrics = module_metric.MetricTracker(
            'loss', 'mae', 'mse', *[m for m in self.metric_ftns],
            writer=self.writer)
        
        self.ordinal_loss = corn_loss
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            target, image = data['lab'].to(self.device), data['img'].to(self.device)
            add_data = data['add'].to(self.device)
            grade = data['raw_grades'].to(self.device)
            ...
            ord_loss = self.ordinal_loss(add_output, grade, num_classes=8)

            loss = self.criterion.ordinal_forward(output, target, ord_loss)
            ...
            self.train_metrics.update('base_loss', self.criterion.base_loss.item())
            self.train_metrics.update('ordi_loss', self.criterion.pi_loss.item())
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(self.val_data_loader):
                ...
                target, image = data['lab'].to(self.device), data['img'].to(self.device)
                grades = data['raw_grades'].to(self.device)
                add_data = data['add'].to(self.device)

                output, add_output = self.model(image, add_data, test_return=True)
                
                target_epoch = torch.cat((target_epoch, target))
                output_epoch = torch.cat((output_epoch, output))
                predicted_grades = corn_label_from_logits(add_output).float()

                mse_loss = torch.sum((predicted_grades - grades)**2) / grades.size(0)
                mae_loss = torch.sum(torch.abs(predicted_grades - grades)) / grades.size(0)
                loss = self.criterion(softmax(output, dim=1), target)

                self.writer.set_step((epoch - 1) * len(self.val_data_loader) + batch_idx, 'valid')
                self.valid_metrics.update('loss', loss.item())
                self.valid_metrics.update('mae', mae_loss.item())
                self.valid_metrics.update('mse', mse_loss.item())
    ...
    def test(self, test_loader=None):
        ...
        test_metrics = module_metric.MetricTracker('loss', 'mae', 'mse', *[m for m in self.metric_ftns])
        test_metrics.reset()
        ...
        with torch.no_grad():
            ...
            grade_epoch = torch.tensor([])
            idx_epoch = torch.tensor([])
            predicted_grades_epoch = torch.tensor([])
            ...
            for data in test_loader:
                ...
                # Get the embedding, grade and uncertainty for visualization
                embedding_epoch = torch.cat((embedding_epoch, embedding))
                grade_epoch = torch.cat((grade_epoch, data['raw_grades']))
                uncertainty_epoch = torch.cat((uncertainty_epoch, data['uncertainty']))
                idx_epoch = torch.cat((idx_epoch, data['idx']))
                predicted_grades_epoch = torch.cat((predicted_grades_epoch, predicted_grades.cpu()))
                ...
                test_metrics.update('loss', loss.item())
                test_metrics.update('mae', mae_loss.item())
                test_metrics.update('mse', mse_loss.item())
        ...
        with open(self.log_dir / save_name, 'wb') as fild_handle:
            ...
            np.save(fild_handle, uncertainty_epoch.cpu().numpy())
            np.save(fild_handle, idx_epoch.cpu().numpy())
            np.save(fild_handle, predicted_grades_epoch.cpu().numpy())
